...
class QueryBuilder {
    ...
    setPivotConfig(index, columns, values, aggfunc = 'sum') {
        this.state.pivot = { index, columns, values, aggfunc };
    }

    clearPivotConfig() {
        delete this.state.pivot;
    }
    ...
    saveQuery(name) {
        ...
        this.state.savedQueries.push(query);
        this.saveToLocalStorage();
        return query;
    }

    loadQuery(name) {
        const query = this.state.savedQueries.find(q => q.name === name);
        if (query) {
            const { config } = query;
            this.state.filters = [...config.filters];
            this.state.groupBy = [...config.group_by];
            this.state.orderBy = [...config.order_by];
            this.state.calculatedFields = [...config.calculated_fields];
            this.state.selectedFields = [...config.selected_fields];
            this.state.pivot = config.pivot ? { ...config.pivot } : null;
            return true;
        }
        return false;
    }
    ...
    saveToLocalStorage() {
        localStorage.setItem('engisuite_saved_queries', JSON.stringify(this.state.savedQueries));
    }

    loadFromLocalStorage() {
        const saved = localStorage.getItem('engisuite_saved_queries');
        if (saved) {
            this.state.savedQueries = JSON.parse(saved);
        }
    }
