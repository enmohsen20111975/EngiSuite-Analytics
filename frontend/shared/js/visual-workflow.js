// Load library data from backend API instead of local files
async function fetchLibraryFile(fileName) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            return { workflows }; // Return in same format as JSON file
        }
    } catch (error) {
        console.warn('Backend API unavailable, falling back to local files');
    }
    
    // Fallback to local files if API fails
    const DATA_SOURCES = ['shared/data', './shared/data', '../shared/data', '/shared/data', '.', ''];
...
const VisualWorkflow = {
    ...
    initCollapsedCategories: function() {
        ...
        Object.keys(Calculators.modules).forEach((moduleKey) => {
            this.collapsedCategories[moduleKey] = true;
        });
    },
    ...
    populateComponents: function() {
        ...
        const moduleEntries = Object.entries(Calculators.modules).filter(([moduleKey]) => {
            return this.activeCategory === 'all' || this.activeCategory === moduleKey;
        });

        moduleEntries.forEach(([moduleKey, module]) => {
            ...
            html += `
                <div class="component-category" style="margin-bottom: 1rem;">
                    <h4 class="category-title" 
                        ...
                        style="font-size: 0.9rem; color: var(--text-secondary); margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.05em; cursor: pointer; display: flex; align-items: center; gap: 6px; padding: 8px; border-radius: 6px; transition: all 0.2s;"
                        onmouseover="this.style.background='var(--bg-tertiary)'; this.style.color='var(--primary-color)';"
                    ...
                    <div class="category-items" style="display: ${isCollapsed ? 'none' : 'block'}; padding-left: 8px;">
                         ${calcs.map(([calcName, calc]) => `
                            <div class="draggable-component glass" 
                                ...
                                class="draggable-component p-3 mb-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 cursor-grab hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-brand-blue transition-all"
                            >
                                <div class="font-semibold mb-1">${calc.nameKey ? (i18n.getTranslation(calc.nameKey) || calcName) : calcName}</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">${calc.desc || ''}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
    ...
    loadLibraries: async function() {
        try {
            // Load workflows from API or fallback
            ...
            this.workflowLibrary = [];
        }

        try {
            // Load examples from API or fallback
            ...
            this.examplesLibrary = [];
        }

        this.workflowExamples = this.workflowLibrary.map((workflow) => {
            const toPlaceholders = (items = []) => items.reduce((acc, item) => {
                acc[item] = '-';
                return acc;
            }, {});
            ...
            return {
                ...
                inputs: toPlaceholders(workflow.inputs),
                outputs: toPlaceholders(workflow.outputs)
            };
        });
    ...
    populateExamplesMenu: function() {
        ...
        const featuredItems = Object.entries(this.exampleWorkflows).map(([key, example]) => {
            const matches = matchesSearch(example.name) || matchesSearch(example.description);
            if (!matches) return '';
            return `
                <div class="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 hover:border-brand-blue transition-all" onclick="VisualWorkflow.loadExample('${key}')">
                    <h5 class="font-medium text-slate-800 dark:text-slate-100 mb-1">${example.name}</h5>
        ...
        featuredContainer.innerHTML = featuredItems ? 
            `<div class="space-y-2"><h5 class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2"><i class="fas fa-star text-amber-500"></i> Featured Examples</h5>${featuredItems}</div>` : 
            '';

        // Workflows Library - Grouped by Domain
        ...
        workflowItems.forEach((workflow) => {
            const domain = workflow.domain || 'general';
            if (!groupedWorkflows[domain]) {
                groupedWorkflows[domain] = [];
            }
            groupedWorkflows[domain].push(workflow);
        });

        let workflowsHtml = '';
        if (Object.keys(groupedWorkflows).length > 0) {
            workflowsHtml = '<div class="space-y-2"><h5 class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2"><i class="fas fa-project-diagram text-brand-blue"></i> Workflows Library</h5>';
            Object.entries(groupedWorkflows).forEach(([domain, workflows]) => {
                workflowsHtml += `<div class="text-xs font-semibold text-brand-blue uppercase tracking-wider px-2 py-1 bg-blue-50 dark:bg-blue-900/30 rounded inline-block mt-2 mb-1">${domain.toUpperCase()}</div>`;
                workflows.forEach((workflow) => {
                    workflowsHtml += `
                        <div class="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 hover:border-brand-blue transition-all" onclick="VisualWorkflow.showWorkflowDetails('${workflow.id}')">
                            <h5 class="font-medium text-slate-800 dark:text-slate-100 mb-1">${workflow.title}</h5>
                            <p class="text-sm text-slate-500 dark:text-slate-400">${workflow.description}</p>
                        </div>
                    `;
                });
            });
            ...
            workflowsHtml = '<div class="p-3 text-slate-500 dark:text-slate-400 text-sm">No workflows found.</div>';
        }
        workflowsContainer.innerHTML = workflowsHtml;

        // Examples Library - Grouped by Domain
        const mergedExamples = [...this.examplesLibrary, ...this.workflowExamples];
        const exampleItems = mergedExamples.filter((example) => {
        ...
        exampleItems.forEach((example) => {
            ...
            if (!groupedExamples[domain]) {
                groupedExamples[domain] = [];
            }
            groupedExamples[domain].push(example);
        });

        let examplesHtml = '';
        if (Object.keys(groupedExamples).length > 0) {
            examplesHtml = '<div class="space-y-2"><h5 class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2"><i class="fas fa-calculator text-emerald-500"></i> Calculation Examples</h5>';
            Object.entries(groupedExamples).forEach(([domain, examples]) => {
                examplesHtml += `<div class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider px-2 py-1 bg-emerald-50 dark:bg-emerald-900/30 rounded inline-block mt-2 mb-1">${domain.toUpperCase()}</div>`;
                examples.forEach((example) => {
                    examplesHtml += `
                        <div class="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 hover:border-brand-blue transition-all" onclick="VisualWorkflow.showExampleDetails('${example.id}')">
                            <h5 class="font-medium text-slate-800 dark:text-slate-100 mb-1">${example.title}</h5>
                            <p class="text-sm text-slate-500 dark:text-slate-400">${example.scenario}</p>
                        </div>
                    `;
                });
            });
            ...
            examplesHtml = '<div class="p-3 text-slate-500 dark:text-slate-400 text-sm">No calculation examples found.</div>';
        }
    ...
    onComponentTouchEnd: function(e) {
        ...
        if (this.touchDraggingComponent.element) {
            ...
            this.touchDraggingComponent.element.style.background = 'var(--card-bg)';
            this.touchDraggingComponent.element.style.borderColor = 'var(--border-color)';
        }
    ...
    setCategoryFilter: function(category) {
        ...
        document.querySelectorAll('.workflow-filter').forEach((button) => {
...
class CalculatorNode {
    ...
    draw(ctx, isSelected, connections = []) {
        ...
        ctx.strokeStyle = isSelected ? '#3b82f6' : (this.hasError ? '#ef4444' : '#cbd5e1');
        ...
        this.inputPorts.forEach((port, i) => {
            ...
            ctx.fillStyle = isConnected ? '#0f172a' : '#334155';
