...
PaymentService.prototype.validatePromoCode = function(code, planId, isAnnual) {
    var _this = this;
    return new Promise(function(resolve, reject) {
        fetch(_this.baseUrl + '/payments/validate-promo', {
            method: 'POST',
            headers: {
        ...
        .then(function(response) {
            if (!response.ok) {
                response.json().then(function(error) {
                    reject(new Error(error.detail || 'Failed to validate promo code'));
                }).catch(function() {
                    reject(new Error('Failed to validate promo code'));
                });
            } else {
