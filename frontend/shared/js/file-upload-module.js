...
export const fileUploadModule = {
    ...
    init() {
        ...
        // Pivot Table controls event listeners
        document.getElementById('generatePivotBtn').addEventListener('click', this.generatePivot.bind(this));
    },
    ...
    displayProcessedDataInAnalysisTab(results) {
        ...
        // Pivot selects
        const pivotRowsSelect = document.getElementById('pivotRowsSelect');
        const pivotColsSelect = document.getElementById('pivotColsSelect');
        const pivotValuesSelect = document.getElementById('pivotValuesSelect');
        ...
        pivotRowsSelect.innerHTML = '';
        pivotColsSelect.innerHTML = '';
        pivotValuesSelect.innerHTML = '<option value="">Select Value</option>';
        ...
        if (firstResult.data && firstResult.data.length > 0) {
            ...
            columns.forEach(col => {
                ...
                optRow.value = col;
                optRow.textContent = col;
                pivotRowsSelect.appendChild(optRow);

                const optCol = document.createElement('option');
                optCol.value = col;
                optCol.textContent = col;
                pivotColsSelect.appendChild(optCol);

                const optVal = document.createElement('option');
                optVal.value = col;
                optVal.textContent = col;
                pivotValuesSelect.appendChild(optVal);
            });
    ...
    async generatePivot() {
        const rows = Array.from(document.getElementById('pivotRowsSelect').selectedOptions).map(opt => opt.value);
        const cols = Array.from(document.getElementById('pivotColsSelect').selectedOptions).map(opt => opt.value);
        const values = document.getElementById('pivotValuesSelect').value;
        const aggfunc = document.getElementById('pivotAggFunc').value;
        const container = document.getElementById('pivotResultContainer');

        if (!this.currentAnalysisData || rows.length === 0 || cols.length === 0 || !values) {
            this.showError('Please select rows, columns, and values for the pivot table.');
            return;
        }

        this.showProgressOverlay('Generating Pivot Table...');

        try {
            const query = {
                data: JSON.stringify(this.currentAnalysisData),
                query: {
                    pivot: {
                        index: rows,
                        columns: cols,
            ...
            const response = await analyticsService.executeQuery(query.data, query.query);
            if (response.success) {
                this.renderPivotResult(JSON.parse(response.results));
                this.showSuccess('Pivot table generated successfully!');
            } else {
                this.showError(response.error);
            }
        } catch (error) {
            this.showError('Pivot generation failed: ' + error.message);
        } finally {
            this.hideProgressOverlay();
        }
    },

    renderPivotResult(data) {
        const container = document.getElementById('pivotResultContainer');
        if (!data || data.length === 0) {
            container.innerHTML = '<p>No results for this pivot configuration.</p>';
            return;
        }
    ...
    // ==================== Enhanced File Processing for Larger Datasets ====================
    
    async processLargeFile(file, chunkSize = 10000) {
        // Validate file size
        ...
        this.showProgressOverlay(`Processing large file: ${file.name} (${this.formatFileSize(file.size)})`);
        ...
        try {
            ...
            if (fileExtension === 'xlsx' || fileExtension === 'xls') {
                // Get sheets first
                const sheetsResponse = await analyticsService.getExcelSheets(fileContent, file.name);
                if (sheetsResponse.success && sheetsResponse.sheets) {
                    this.renderSheetsSelection(sheetsResponse.sheets, this.uploadedFiles.length);
                    this.showSheetsSection();
                }
            } else {
                // For CSV/JSON files, process directly
                await this.processFileChunk(fileContent, file.name, null);
            }
        } catch (error) {
    ...
    async processFileChunk(fileContent, filename, sheetName = null) {
        try {
            const response = await analyticsService.processFile(fileContent, filename, sheetName);
            if (response.success) {
                const data = JSON.parse(response.data);
                
                // Store in IndexedDB for large datasets
                if (data.length > 5000) {
                    await analyticsAdvanced.saveDataset(`${filename}${sheetName ? ' - ' + sheetName : ''}`, data);
                }
            ...
            this.showError(`Error processing file chunk: ${error.message}`);
    ...
    validateFile(file) {
        ...
        if (!validExtensions.includes(extension)) {
            this.showError(`Invalid file type: ${extension}. Supported types: ${validExtensions.join(', ')}`);
            return false;
        }

        if (file.size > this.maxFileSize) {
            this.showError(`File too large. Maximum size: ${this.formatFileSize(this.maxFileSize)}`);
            return false;
        }
    ...
    async loadFromIndexedDB(datasetId) {
        try {
            await analyticsAdvanced.initDB();
            const dataset = await analyticsAdvanced.getDataset(datasetId);
            
            if (dataset) {
                this.currentAnalysisData = dataset.data;
                this.renderDataPreview(dataset.data);
                this.populateAxisSelects(Object.keys(dataset.data[0]));
            }
        } catch (error) {
            this.showError(`Error loading from storage: ${error.message}`);
        }
    },
    ...
    async saveToIndexedDB(name, data) {
        try {
            await analyticsAdvanced.initDB();
            const result = await analyticsAdvanced.saveDataset(name, data);
            this.showSuccess(`Dataset saved to storage: ${name}`);
            return result;
        } catch (error) {
            this.showError(`Error saving to storage: ${error.message}`);
            return null;
        }
    },
    ...
    async clearIndexedDB() {
        try {
            await analyticsAdvanced.initDB();
            await analyticsAdvanced.clearAllDatasets();
            this.showSuccess('All data cleared from storage');
        } catch (error) {
            this.showError(`Error clearing storage: ${error.message}`);
        }
    },
